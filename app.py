...
def page_overview(team: dict, all_teams: dict):
    ...
    <div style="display:flex; align-items:center; gap:16px; margin-bottom:1.2rem;">
        ...
        <div style="margin-left:auto;text-align:right;">
            ...
            <div style="font-size:12px;color:#888;">%V {team["pct"]:.3f} · #{team["conf_rank"]} {_conf_pt}</div>
    ...
    with col_right:
        ...
        opp_map = {}
        gids = [g.get("game_id") for g in team["last_games"][:10] if g.get("game_id")]
        if gids:
            ...
            games_data.append(
                {
                    ...
                    "W/L": g["wl"],
                    "PTS": g["pts"],
                    "PTS Sof.": opp_map.get(g.get("game_id"), "—"),
                    "REB": g["reb"],
                    "AST": g["ast"],
                    "STL": g.get("stl", 0),
                    "BLK": g.get("blk", 0),
    ...
    if injuries:
        ...
        for inj in injuries:
            status = inj["status"]
            if status == "Out":
                badge = '<span style="background:#C62828;color:#fff;padding:2px 8px;border-radius:4px;font-size:0.85em">Fora</span>'
            else:
                badge = '<span style="background:#F9A825;color:#000;padding:2px 8px;border-radius:4px;font-size:0.85em">Incerto</span>'
            ...
            h2h_rows.append(
                {
                    ...
                    "R": g["wl"],
                    "Pts": g["pts"],
                    "Pts Sof.": opp_h2h.get(g.get("game_id"), "—"),
                    "Reb": g["reb"],
                    "Ast": g["ast"],
                    "Stl": g["stl"],
                    "Blk": g["blk"],
...
def page_players():
    ...
    if "_fver" not in st.session_state:
        st.session_state["_fver"] = 0
